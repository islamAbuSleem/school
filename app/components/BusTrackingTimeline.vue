...
<template>
  <div class="mt-12 bg-[#F8FAFC] rounded-[48px] p-12 shadow-inner ring-1 ring-slate-100 relative group overflow-hidden">
    ...
    <div class="flex items-start justify-between relative px-10">
      <!-- Connection Line -->
      <div class="absolute top-[11px] left-20 right-20 h-0.5 bg-slate-200">
        <div class="h-full bg-indigo-600 transition-all duration-1000" style="width: 50%"></div>
      </div>

      <div v-for="stop in stops" :key="stop.name" class="flex flex-col items-center relative z-10 w-40">
